...
public class ExceptionController extends ResponseEntityExceptionHandler {
    ...
    public final ResponseEntity<ApiException> handleStockAPIException(StockAPIException ex) {
        Gson g = new Gson();
        ApiException apiException = g.fromJson(ex.getMessage().substring(7,ex.getMessage().length()-1), ApiException.class);

        if(apiException.getMessage().contains("QUANTITY NOT VALID STOCK AVAILABLE")){
            // call le service
            bookService.OrderBook("not implemeted");
            return new ResponseEntity<>(new ApiException(new Date(), "NOT STOCK AVAILABLE", apiException.getDetails()),  HttpStatus.BAD_REQUEST);
        }
